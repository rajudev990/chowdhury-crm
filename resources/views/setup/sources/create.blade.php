@extends('layouts.app')
@section('title', isset($source) ? 'Edit Source' : 'Create Source')
...
<div class="container mx-auto mt-3">
    <div class="max-w-xl mx-auto bg-white rounded-lg shadow-xl border border-gray-200">
        <h5 class="bg-[#1A3A66] font-semibold mb-6 p-2 text-xl text-white">
            {{ isset($source) ? 'Edit Source' : 'Create Source' }}
        </h5>
        ...
        <form action="{{ isset($source) ? route('sources.update', $source->id) : route('sources.store') }}" method="POST" class="p-6">
            @csrf
            @if(isset($source)) @method('PUT') @endif

            <!-- Source Name -->
            <div class="mb-6">
                <label for="name" class="block text-[#1A3A66] font-semibold mb-2">Source Name</label>
                <input type="text" id="name" name="name" value="{{ old('name', $source->name ?? '') }}" 
                ...
                <div class="flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="status" name="status" value="1" 
                            class="sr-only peer" 
                            {{ old('status', $source->status ?? 1) ? 'checked' : '' }}>
                        <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#1A3A66]"></div>
                        <span class="ml-3 text-sm font-medium text-gray-700">Active</span>
                    </label>
                </div>
                <p class="text-gray-500 text-sm mt-1">Toggle to set source as active or inactive</p>
            </div>
            ...
            <div class="flex justify-between items-center">
                ...
                <button type="submit" 
                    ...
                    {{ isset($source) ? 'Update Source' : 'Create Source' }}
                </button>
            </div>
        </form>
    </div>
