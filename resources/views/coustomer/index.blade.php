                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <!-- Type Badge with Letter Initial Only -->
                                        <button onclick="openTypeChangeModal({{ $item->id }}, '{{ $item->type }}')"
                                            class="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm transition-all hover:shadow-lg hover:scale-110
                                                    {{ $item->type === 'admin' ? 'bg-[#1A3A66] hover:bg-[#1A3A66]' : '' }}
                                                    {{ $item->type === 'user' ? 'bg-[#1A3A66] hover:bg-[#1A3A66]' : '' }}
                                                    {{ $item->type === 'customer' ? 'bg-[#1A3A66] hover:bg-[#1A3A66]' : '' }}"
                ...
                <div id="statusSidebar"
                    ...
                    <form id="userForm" method="POST" action="{{ route('customers.store') }}" class="p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div class="floating-label-group">
                                <select id="userAssignedId" name="assigned_id" onchange="setUserType(this.value)" required>
                                    <option value=""></option>
                                    <option value="admin" data-type="admin">Admin</option>
                                    <option value="user" data-type="user">User</option>
                                    <option value="customer" data-type="customer">Customer</option>
                                </select>
                                <label>Assigned <span class="required">*</span></label>
                            </div>
                            <!-- Hidden field for type -->
                            <input type="hidden" id="userType" name="type" value="">
                        </div>
                ...
                <div id="typeChangeModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden flex items-center justify-center" onclick="closeTypeChangeModal()">
                    <div class="bg-white rounded-xl shadow-2xl p-6 w-96 mx-4 transform transition-all" onclick="event.stopPropagation()">
                        ...
                        <div class="flex items-center justify-between mb-5">
                            <h3 class="text-lg font-bold text-[#1A3A66]">Change User Type</h3>
                            <button onclick="closeTypeChangeModal()" class="w-7 h-7 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition">
                                <i class="fa-solid fa-times"></i>
                            </button>
                        </div>

                        <!-- Current User Info -->
                        <div id="currentUserInfo" class="flex items-center gap-3 p-3 bg-[#1A3A66] rounded-lg mb-5 text-white">
                            <!-- This will be populated by JavaScript -->
                        </div>

                        <form id="typeChangeForm" method="POST">
                            @csrf
                            @method('PATCH')

                            <div class="mb-5">
                                <label class="block text-sm font-semibold text-[#1A3A66] mb-2">Select New Type</label>
                                <div class="space-y-2">
                                    <!-- Admin Option -->
                                    <label class="flex items-center gap-3 p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-[#1A3A66] hover:bg-gray-50 transition-all group">
                                        <input type="radio" name="type" value="admin" class="w-4 h-4 text-[#1A3A66] focus:ring-[#1A3A66]" onchange="updateModalIcon('admin')">
                                        <div class="w-9 h-9 bg-[#1A3A66] rounded-full flex items-center justify-center text-white font-bold shadow-sm group-hover:scale-105 transition-transform">
                                            A
                                        </div>
                                        <span class="font-medium text-[#1A3A66]">Admin</span>
                                    </label>

                                    <!-- User Option -->
                                    <label class="flex items-center gap-3 p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-[#1A3A66] hover:bg-gray-50 transition-all group">
                                        <input type="radio" name="type" value="user" class="w-4 h-4 text-[#1A3A66] focus:ring-[#1A3A66]" onchange="updateModalIcon('user')">
                                        <div class="w-9 h-9 bg-[#1A3A66] rounded-full flex items-center justify-center text-white font-bold shadow-sm group-hover:scale-105 transition-transform">
                                            U
                                        </div>
                                        <span class="font-medium text-[#1A3A66]">User</span>
                                    </label>

                                    <!-- Customer Option -->
                                    <label class="flex items-center gap-3 p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-[#1A3A66] hover:bg-gray-50 transition-all group">
                                        <input type="radio" name="type" value="customer" class="w-4 h-4 text-[#1A3A66] focus:ring-[#1A3A66]" onchange="updateModalIcon('customer')">
                                        <div class="w-9 h-9 bg-[#1A3A66] rounded-full flex items-center justify-center text-white font-bold shadow-sm group-hover:scale-105 transition-transform">
                                            C
                                        </div>
                                        <span class="font-medium text-[#1A3A66]">Customer</span>
                                    </label>
                                </div>
                            </div>

                            <div class="flex gap-3">
                                <button type="button" onclick="closeTypeChangeModal()"
                                    class="flex-1 px-4 py-2.5 border-2 border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-all">
                                    Cancel
                                </button>
                                <button type="submit"
                                    class="flex-1 px-4 py-2.5 bg-[#1A3A66] text-white font-medium rounded-lg hover:bg-[#163158] shadow-md hover:shadow-lg transition-all">
                                    Update
                                </button>
                            </div>
                        </form>
...
<script>
    ...
    function setUserType(assignedValue) {
        const typeField = document.getElementById('userType');
        if (typeField) {
            typeField.value = assignedValue;
            console.log('Type set to:', assignedValue);
        } else {
    ...
    function loadUserData(user) {
        ...
        document.getElementById('userAssignedId').value = user.type ?? ''; // পরিবর্তন করুন
        document.getElementById('userType').value = user.type ?? ''; // এই লাইন যোগ করুন
...
<script>
    function openTypeChangeModal(userId, currentType) {
        ...
        const userName = row.querySelector('td:nth-child(2)')?.textContent.trim() || 'User';

        // Get type letter
        let typeLetter = '?';
        let typeFullName = currentType || 'Unknown';
        if (currentType === 'admin') typeLetter = 'A';
        else if (currentType === 'user') typeLetter = 'U';
        else if (currentType === 'customer') typeLetter = 'C';

        // Update user info in modal
        userInfo.innerHTML = `
        <div id="modalTypeIcon" class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center text-white font-bold text-lg border-2 border-white/30 flex-shrink-0">
            ${typeLetter}
        </div>
        <div class="flex-1">
            <p class="font-semibold text-base">${userName}</p>
            <p class="text-xs text-white/80">Current: <span class="capitalize font-medium">${typeFullName}</span></p>
        </div>
    `;
    ...
    function updateModalIcon(selectedType) {
        const iconDiv = document.getElementById('modalTypeIcon');
        if (!iconDiv) return;

        let typeLetter = '?';
        if (selectedType === 'admin') typeLetter = 'A';
        else if (selectedType === 'user') typeLetter = 'U';
        else if (selectedType === 'customer') typeLetter = 'C';

        iconDiv.textContent = typeLetter;
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                submitButton.disabled = false;
                submitButton.textButton.textContent = originalText;
            }
        });
    });
</script>
