        ...
        <ul class="flex -mb-px" id="tabs">
            <li class="mr-1">
                <a href="#" data-tab="profile"
                    class="tab-link inline-block py-2 px-4 text-blue-600 font-semibold border-b-2 border-blue-600">Profile</a>
            </li>
            <li class="mr-1">
                <a href="#" data-tab="attachment"
                    class="tab-link inline-block py-2 px-4 text-gray-600 border-b-2 border-transparent hover:border-blue-500">Attachment</a>
            </li>
            <li class="mr-1">
                <a href="#" data-tab="note"
                    class="tab-link inline-block py-2 px-4 text-gray-600 border-b-2 border-transparent hover:border-blue-500">Note</a>
            </li>
            <li class="mr-1">
                <a href="#" data-tab="task"
                    class="tab-link inline-block py-2 px-4 text-gray-600 border-b-2 border-transparent hover:border-blue-500">Task</a>
            </li>
            <li class="mr-1">
                <a href="#" data-tab="email"
                    class="tab-link inline-block py-2 px-4 text-gray-600 border-b-2 border-transparent hover:border-blue-500">Email</a>
            </li>
        </ul>
    </div>
    ...
    <div class="p-6 bg-white rounded-b-lg mt-2">
            ...
            <div class="bg-white border border-gray-200 rounded-lg shadow-sm mb-6">
                ...
                <div class="flex justify-between items-center p-6 border-b border-gray-200">
                    ...
                    <div class="flex gap-3">
                        <button
                            class="flex items-center gap-2 bg-[#3B5998] hover:bg-[#2d4373] text-white px-5 py-2.5 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md">
                        ...
                        <button
                            class="flex items-center gap-2 bg-gray-700 hover:bg-gray-800 text-white px-5 py-2.5 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md">
                ...
                <div class="p-8">
                    <div class="grid grid-cols-12 gap-8">
                        ...
                        <div class="col-span-3 flex flex-col items-center">
                            <div
                                class="w-44 h-44 rounded-full bg-[#3B5998] flex items-center justify-center mb-4 shadow-lg">
                        ...
                        <div class="col-span-4 space-y-5">
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Phone
                                    Number</label>
                                <p class="text-gray-800 font-medium">{{ $lead->phone ?? '–' }}</p>
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Additional
                                    Phone</label>
                                <p class="text-gray-800 font-medium">{{ $lead->additional_phone ?? '–' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Date
                                    of Birth</label>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Created
                                    Date</label>
                                <p class="text-gray-800 font-medium">{{ $lead->created_at ? $lead->created_at->format('M
                                    d, Y') : '–' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Date
                                    of Contact</label>
                                <p class="text-gray-800 font-medium">
                                    {{ $lead->date_of_contact ? \Carbon\Carbon::parse($lead->date_of_contact)->format('M
                                    d, Y') : '–' }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="col-span-3 space-y-5">
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">City</label>
                                <p class="text-gray-800 font-medium">{{ $lead->city ?? '–' }}</p>
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Country</label>
                                <p class="text-gray-800 font-medium">{{ $lead->country ?? '–' }}</p>
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Preferred
                                    Country</label>
                                <p class="text-gray-800 font-medium">{{ $lead->preferred_country ?? '–' }}</p>
                            </div>
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Address</label>
                                <p class="text-gray-800 font-medium">{{ $lead->address ?? '–' }}</p>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Lead
                                    Status</label>
                                <span
                                    class="inline-block bg-blue-50 text-blue-700 px-4 py-1.5 rounded-full text-sm font-medium border border-blue-200">
                        ...
                        <div class="col-span-2 space-y-5">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">How
                                    Did Hear About Us?</label>
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <div
                                        class="w-10 h-10 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 flex items-center justify-center shadow-md">
                                        <span class="text-white text-sm font-bold">
                                            {{ strtoupper(substr($lead->assignedUser->name, 0, 1)) }}
                                        </span>
                                    </div>
                                    <span class="text-gray-800 font-medium text-sm">{{ $lead->assignedUser->name
                                        }}</span>
                                </div>
                                @else
                    ...
                    <div class="mt-8 pt-8 border-t border-gray-200">
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Personal
                            Information Note</label>
                        ...
                        <div class="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-5 border border-purple-100">
                            <div class="flex items-start justify-between mb-4">
                                <div>
                                    ...
                                    <p class="text-gray-600 font-medium text-sm">{{ $exam->institute_name ?? 'N/A' }}
                                    </p>
                                </div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-5 border border-blue-100">
                            <div class="flex items-start justify-between mb-4">
                                ...
                                <span
                                    class="bg-blue-600 text-white px-4 py-1.5 rounded-full text-sm font-semibold uppercase">
                                    {{ $test->test_type ?? 'N/A' }}
                                </span>
                            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg shadow-sm mb-6">
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div
                            class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-5 border border-green-100">
                            <div class="flex items-start justify-between mb-4">
                                <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div class="bg-white rounded-lg p-3 shadow-sm">
                                    ...
                                    <p class="text-gray-800 font-medium">
                                        {{ $job->joining_date ? \Carbon\Carbon::parse($job->joining_date)->format('M d,
                                        Y') : '–' }}
                                    </p>
                                </div>
                                <div class="bg-white rounded-lg p-3 shadow-sm">
                                    ...
                                    <p class="text-gray-800 font-medium">
                                        {{ $job->end_date ? \Carbon\Carbon::parse($job->end_date)->format('M d, Y') :
                                        'Present' }}
                                    </p>
                                </div>
                            </div>
        ...
        <div id="attachment" class="tab-content hidden">
            <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                ...
                <div id="dropZone"
                    class="border-2 border-dashed border-gray-300 p-6 rounded-lg text-center mb-4 cursor-pointer hover:bg-gray-50">
                    Drag & Drop files here or click to select
                </div>
                <input type="file" id="attachmentFile" class="hidden">
                <input type="hidden" id="user_id" value="{{ $lead->id }}">

                <input type="text" id="attachmentDescription" placeholder="Attachment description"
                    class="border px-3 py-2 rounded w-full mb-3">
                <button id="uploadAttachment"
                    class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 mb-4">Upload</button>
                ...
                <table class="w-full text-left border">
                    ...
                    <tbody id="attachmentTable">
                        ...
                        <tr data-id="{{ $attachment->id }}">
                            <td class="p-3"><a href="{{ asset('storage/'.$attachment->file_path) }}" target="_blank">{{
                                    $attachment->file_name }}</a></td>
                            <td class="attachment-description p-3">{{ $attachment->description }}</td>
                            <td class="flex gap-2 p-3">
                                <button class="editAttachment bg-yellow-500 text-white px-2 py-1 rounded"><i class="fas fa-pencil text-lg"></i></button>
                                <button class="deleteAttachment bg-red-500 text-white px-2 py-1 rounded"><i class="fas fa-trash text-lg"></i></button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div id="editAttachmentModal"
            class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white p-6 rounded-lg w-full max-w-md">
                <h3 class="text-lg font-bold mb-4">Edit Attachment</h3>
                ...
                <div class="mb-3">
                    ...
                    <div id="editDropZone"
                        ...
                        Drag & Drop or Click to select new file
                    </div>
                    <input type="file" id="editAttachmentFile" class="hidden">
                    <p id="editFileName" class="text-gray-700 mt-1"></p>
                </div>

                <div class="mb-3">
                    <label class="block mb-1 font-medium">Description</label>
                    <input type="text" id="editAttachmentDescription" class="border px-3 py-2 rounded w-full">
                </div>

                <div class="flex justify-end gap-2">
                    <button id="closeEditAttachment"
                        class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Cancel</button>
                    <button id="saveEditAttachment"
...
<script>
    ...
    $('#dropZone').click(function(){ $('#attachmentFile').trigger('click'); });

    $('#attachmentFile').change(function(e){
        fileToUpload = e.target.files[0];
        $('#dropZone').text(fileToUpload ? fileToUpload.name : 'Drag & Drop files here or click to select');
    });

    $('#dropZone').on('dragover', function(e){ e.preventDefault(); $(this).addClass('bg-gray-100'); });
    $('#dropZone').on('dragleave', function(e){ e.preventDefault(); $(this).removeClass('bg-gray-100'); });
    $('#dropZone').on('drop', function(e){
        e.preventDefault(); $(this).removeClass('bg-gray-100');
        fileToUpload = e.originalEvent.dataTransfer.files[0];
        $('#dropZone').text(fileToUpload.name);
    });

    // Upload Attachment
    $('#uploadAttachment').click(function(){
        if(!fileToUpload) return Swal.fire({
            toast:true,
            position:'top-end',
            icon:'error',
            ...
            success:function(res){
                if(res.success){
                    $('#attachmentTable').append(`
                        <tr data-id="${res.attachment.id}">
                            <td><a href="/storage/${res.attachment.file_path}" target="_blank">${res.attachment.file_name}</a></td>
                            <td class="attachment-description">${res.attachment.description}</td>
                            <td class="flex gap-2">
                                <button class="editAttachment bg-yellow-500 text-white px-2 py-1 rounded"><i class="fas fa-pencil text-lg"></i></button>
                                <button class="deleteAttachment bg-red-500 text-white px-2 py-1 rounded"><i class="fas fa-trash text-lg"></i></button>
                            </td>
    ...
    // Edit Attachment Modal
    var editFileToUpload = null;

    $(document).on('click', '.editAttachment', function(){
        var row = $(this).closest('tr');
        var id = row.data('id');
        var description = row.find('.attachment-description').text().trim();
        var fileName = row.find('a').text();

        $('#editAttachmentId').val(id);
        $('#editAttachmentDescription').val(description);
        $('#editFileName').text(fileName);
        $('#editAttachmentModal').removeClass('hidden');
    });

    $('#closeEditAttachment').click(function(){
        $('#editAttachmentModal').addClass('hidden');
        $('#editAttachmentFile').val('');
        $('#editFileName').text('');
        editFileToUpload = null;
    });

    $('#editDropZone').click(function(){ $('#editAttachmentFile').trigger('click'); });
    $('#editAttachmentFile').change(function(e){
        editFileToUpload = e.target.files[0];
        $('#editFileName').text(editFileToUpload.name);
    });
    $('#editDropZone').on('dragover', function(e){ e.preventDefault(); $(this).addClass('bg-gray-100'); });
    $('#editDropZone').on('dragleave', function(e){ e.preventDefault(); $(this).removeClass('bg-gray-100'); });
    $('#editDropZone').on('drop', function(e){
        e.preventDefault(); $(this).removeClass('bg-gray-100');
        editFileToUpload = e.originalEvent.dataTransfer.files[0];
        $('#editFileName').text(editFileToUpload.name);
    });

    $('#saveEditAttachment').click(function(){
        var id = $('#editAttachmentId').val();
        var description = $('#editAttachmentDescription').val();
        var formData = new FormData();
        formData.append('description', description);
        formData.append('_token','{{ csrf_token() }}');
        if(editFileToUpload) formData.append('file', editFileToUpload);
        ...
        $.ajax({
            ...
            success:function(res){
                if(res.success){
                    ...
                    row.find('.attachment-description').text(res.attachment.description);
                    if(res.attachment.file_name){
                        row.find('a').attr('href','/storage/'+res.attachment.file_path).text(res.attachment.file_name);
                    }
                    $('#editAttachmentModal').addClass('hidden');
                    $('#editAttachmentFile').val('');
                    $('#editFileName').text('');
                    ...
                    // toastr.success('Attachment updated');
                }
            }
        });
    });
